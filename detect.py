...
acquire = True
socket.send_string("inizializzazione")
r = socket.recv()
print(r)
	...
	if ((scores[i].any() > min_conf) and (scores[i].any() <= 1.0)):
		...
		if (scores[i] *100) > min_conf*100 :
			...
			if  "ball" in object_name:
				...
				object_heigth = ymax - ymin
				sensors_height = 450
				distance_object = 360
				real_height = 110
				image_height = 480
				focal = (object_heigth*sensors_height*distance_object) / (real_height*image_height)
time.sleep(5)
print(focal)
		...
		if ((scores[i].any() > min_conf) and (scores[i].any() <= 1.0)):
			...
			if (scores[i] *100) > min_conf*100 :
				...
				if  "ball" in object_name:
					...
					cv2.putText(im,str(int(dist)) , (xmin, label_ymin-7), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 2) 
